    ...
    <main className="flex min-h-screen flex-col bg-gray-50">
      ...
      <section className="container mx-auto px-6 md:px-12 lg:px-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {[
          ...
          <div key={index} className="bg-white p-6 shadow-lg rounded-xl text-center border-t-4 border-[#F59E0B]">
            <Image src={item.img} alt={item.title} width={100} height={100} className="mx-auto mb-4 rounded-full" />
            <h2 className="text-xl font-semibold text-[#F59E0B] mb-2">{item.title}</h2>
            <p className="text-gray-600">{item.desc}</p>
          </div>
        ))}
      </section>

      <section className="bg-[#F59E0B] text-white py-12 mt-12">
      ...
      <section className="container mx-auto py-12 px-6 md:px-12 lg:px-20 text-center">
        <h2 className="text-3xl font-bold text-[#F59E0B] mb-4">Get in Touch</h2>
        <p className="text-gray-700 mb-6 max-w-3xl mx-auto">
          Have questions about our products or services? Our team is ready to assist you.
        </p>
        <Link
          href="/contact"
          className="inline-block bg-[#F59E0B] text-white py-3 px-6 rounded-lg text-lg font-semibold shadow-lg hover:bg-orange-600"
        >
          Contact Us
        </Link>
      </section>
